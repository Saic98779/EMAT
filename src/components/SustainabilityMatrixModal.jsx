...
import { useSustainabilityMatrixByAppraisal, useIA } from '../queries'
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth
      ...
      <DialogTitle sx={{
        ...
        <Box sx={{ flexGrow: 1, minWidth: 0 }}>
          <Typography variant="overline" color="text.secondary"
            ...
            GT / SDE Assessment · Sustainability Matrix
          </Typography>
      ...
      <DialogContent sx={{ p: 0, bgcolor: 'background.default' }}>
        ...
        {appraisalUuid && q.error && (
          <Alert severity="warning" sx={{ m: 2 }} variant="outlined">
            {q.error.status === 404
              ...
              : q.error.message || 'Failed to load sustainability record.'}
          </Alert>
        )}
        ...
        {record && (
          <>
            <ScoreHeader score={score} tier={tier} answered={answered} />
            <Box sx={{ p: 2.5 }}>
              <Stack spacing={2}>
                {DIMENSIONS.map((dim) => (
...
function AnswerBadge({ yes, no, unanswered }) {
  if (unanswered) {
    return (
      <SoftPill icon={<RemoveCircleOutlineIcon sx={{ fontSize: 14 }} />} label="—" colorKey="grey" />
    )
  }
  if (yes) {
    return (
      <SoftPill icon={<CheckCircleIcon sx={{ fontSize: 14 }} />} label="Yes" colorKey="success" />
    )
  }
  return (
    <SoftPill icon={<CancelIcon sx={{ fontSize: 14 }} />} label="No" colorKey="error" />
  )
}
